...
`himorime compare` and `himorime ci` refuse settings that could never produce a
verdict. Raise `runs` (or `max_runs` for adaptive runs), or lower
`regression.min_samples`. `himorime run` does not need either.
...
temporary worktree stays in the system temporary directory (`himorime-*`), and
once that directory is deleted, the next `himorime compare` prunes its entry
from the repository.
