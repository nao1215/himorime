...
func defaults() string {
	rows := [][2]string{
		...
		{"regression.latency, cpu, memory: metric", string(config.DefaultMetric)},
		{"regression.latency, cpu, memory: max_percent", fmt.Sprint(config.DefaultMaxPercent)},
...
func configReference() (string, error) {
	...
	sections := []struct {
		...
		{"regression.latency, regression.cpu, regression.memory", []string{"definitions", "latencyRegression"}},
