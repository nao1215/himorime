...
func defaults() string {
	rows := [][2]string{
		...
		{"regression.latency, cpu_total, peak_rss: statistic", string(config.DefaultMetric)},
		{"regression.latency, cpu_total, peak_rss: max_percent", fmt.Sprint(config.DefaultMaxPercent)},
...
func configReference() (string, error) {
	...
	sections := []struct {
		...
		{"benchmarks[].commands.NAME.budget", []string{"definitions", "benchCommand", "properties", "budget"}},
		...
		{"regression.latency, regression.cpu_total, regression.peak_rss", []string{"definitions", "latencyRegression"}},
